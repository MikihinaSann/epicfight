...
public class AzureArmorTransformer extends HumanoidModelTransformer {
	...
	static class GeoModelPartition {
		...
		final AzBone geoBone;

		private GeoModelPartition(PartTransformer<GeoCube> partTransformer, AzBone geoBone) {
	...
	public static void getGeoArmorTexturePath(AnimatedArmorTextureEvent event) {
		...
		if (renderer != null) {
			...
			event.setResultLocation(renderer.rendererPipeline().config().textureLocation(event.getLivingEntity(), event.getItemStack()));
		}
	}
	...
	public SkinnedMesh transformArmorModel(HumanoidModel<?> humanoidModel, LivingEntity livingEntity, ItemStack itemStack, EquipmentSlot equipmentSlot) {
		...
		AzArmorBoneProvider boneProvider = renderer.rendererPipeline().config().boneProvider();

		AzBone headBone = boneProvider.getHeadBone(bakedModel);
		AzBone bodyBone = boneProvider.getBodyBone(bakedModel);
		AzBone rightArmBone = boneProvider.getRightArmBone(bakedModel);
		AzBone leftArmBone = boneProvider.getLeftArmBone(bakedModel);
		AzBone rightLegBone = boneProvider.getRightLegBone(bakedModel);
		AzBone leftLegBone = boneProvider.getLeftLegBone(bakedModel);
		AzBone rightBootBone = boneProvider.getRightBootBone(bakedModel);
		AzBone leftBootBone = boneProvider.getLeftBootBone(bakedModel);

		resetBoneRotation(headBone);
		resetBoneRotation(bodyBone);
		resetBoneRotation(rightArmBone);
		resetBoneRotation(leftArmBone);
		resetBoneRotation(rightLegBone);
		resetBoneRotation(leftLegBone);
		resetBoneRotation(rightBootBone);
		resetBoneRotation(leftBootBone);
	...
	private static void resetBoneRotation(AzBone bone) {
		if (bone != null) {
			bone.setRotX(0);
			bone.setRotY(0);
			bone.setRotZ(0);
	...
	private static void bake(PoseStack poseStack, GeoModelPartition modelpartition, String partName, AzBone geoBone, List<SingleGroupVertexBuilder> vertices, Map<MeshPartDefinition, IntList> indices, List<String> path, PartTransformer.IndexCounter indexCounter, boolean bindPartAnimation) {
		if (geoBone == null) {
			return;
		}
		...
		if (!geoBone.isHidden()) {
			for (GeoCube cube : geoBone.getCubes()) {
				...
				MeshPartDefinition partDefinition = AzureArmorMeshPartDefinition.of(partName);
		...
		if (!geoBone.isHidingChildren()) {
			for (AzBone childBone : geoBone.getChildBones()) {
	...
	public record AzureArmorMeshPartDefinition(String partName, List<String> path, OpenMatrix4f invertedParentTransform, AzBone root) implements MeshPartDefinition {
		public static MeshPartDefinition of(String partName) {
			return new AzureArmorMeshPartDefinition(partName, null, null, null);
		}

		public static MeshPartDefinition of(String partName, List<String> path, OpenMatrix4f invertedParentTransform, AzBone root) {
			return new AzureArmorMeshPartDefinition(partName, path, invertedParentTransform, root);
		}

		public static AzBone getChildBone(AzBone bone, String boneName) {
		...
		public Supplier<OpenMatrix4f> getModelPartAnimationProvider() {
			return this.root == null ? () -> null : () -> {
				...
				AzBone lastBone = bone;
				AzBoneSnapshot boneSnapshot = bone.getInitialAzSnapshot();
		...
		private void progress(AzBone bone, PoseStack poseStack, boolean last) {
			AzBoneSnapshot boneSnapshot = bone.getInitialAzSnapshot();
