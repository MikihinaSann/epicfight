...
public class ProviderConditional
{
	...
	protected final SkillDataKey<?> key;
	protected final InteractionHand hand;
	protected final Predicate<LivingEntityPatch<?>> customFunction;

	private ProviderConditional(ProviderConditionalType type, Style style, Skill skillToCheck, WeaponCategory category, Item weapon, InteractionHand hand, SkillSlot slot, SkillDataKey<?> key, Boolean combination, Predicate<LivingEntityPatch<?>> customFunction, List<ProviderConditional> providerConditionals) {
		this.type = type;
		this.style = style;
	...
	public Boolean test(LivingEntityPatch<?> entityPatch)
	{
		...
		if (type.equals(ProviderConditionalType.DATA_KEY))
		{
			...
			{
				return playerPatch.getSkill(slot).getDataManager().hasData(key) && (Boolean) playerPatch.getSkill(slot).getDataManager().getDataValue(key);
			}
		}
	...
	public Style testConditionalStyle(LivingEntityPatch<?> entityPatch)
	{
		...
		if (type.equals(ProviderConditionalType.DATA_KEY))
		{
			if (entityPatch instanceof PlayerPatch<?> playerPatch)
			{
				if (playerPatch.getSkill(slot).getDataManager().hasData(key) && (Boolean) playerPatch.getSkill(slot).getDataManager().getDataValue(key))
					return style;
			}
		}
		if (type.equals(ProviderConditionalType.SPECIFIC_WEAPON))
...
//	}
	...
	public static class ProviderConditionalBuilder
	{
			...
			try {
				...
				switch (type) {
					...
					case SKILL_EXISTENCE, SKILL_ACTIVATION -> builder
							.setType(type).setWieldStyle(wieldStyle).isVisibleOffHand(visibleOffHand)
							.setSkillToCheck(SkillManager.getSkill(String.valueOf(ResourceLocation.parse(gsonObject.get("skill").getAsString()))))
							.setSlot(SkillSlot.ENUM_MANAGER.get(gsonObject.get("slot").getAsString().toUpperCase()));
					case DATA_KEY -> builder
							.setType(type).setWieldStyle(wieldStyle).isVisibleOffHand(visibleOffHand)
							.setSkillToCheck(SkillManager.getSkill(String.valueOf(ResourceLocation.parse(gsonObject.get("skill").getAsString()))))
							.setKey(SkillDataKeys.REGISTRY.get().getValue(ResourceLocation.parse(gsonObject.get("boolean_key").getAsString())))
							.setSlot(SkillSlot.ENUM_MANAGER.get(gsonObject.get("slot").getAsString().toUpperCase()));
				}
			}
		...
		public ProviderConditionalBuilder setKey(SkillDataKey<?> key) {
			this.key = key;
			return this;
		}
