    ...
    public void postClientTick() {
        ...
        if (this.isFirstPerson() && this.isLerpingFpv()) {
            ...
            if (this.focusingEntity != null && this.lockingOnTarget && !this.isLerpingFpv() && !InputManager.isActionActive(EpicFightInputActions.LOCK_ON_SHIFT_FREELY)) {
                ...
                if (tpsMode) {
                    double cos = (playerToTarget.x * toTarget.x + playerToTarget.y * toTarget.y + playerToTarget.z * toTarget.z);
            ...
            if (
                (playerpatch == null || !playerpatch.getEntityState().inaction() && (!playerpatch.getEntityState().turningLocked() || playerpatch.getEntityState().lookTarget())) &&
